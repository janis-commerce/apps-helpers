...
const formatDeviceDataForUserAgent = (deviceData) => {
	...
	const hasSomeValidValues = keysToCheck.some((key) => isString(deviceData[key]));
	if (!hasSomeValidValues) return {};

	const userAgentParts = [];

	keysToCheck.forEach((key) => {
		const value = isString(deviceData[key]) ? deviceData[key] : `unknown ${key}`;
...
const getHeaders = (params = {}, deviceDataHeaders = {}, customHeaders = {}) => {
	const validUserAgentHeader = formatDeviceDataForUserAgent(deviceDataHeaders);
